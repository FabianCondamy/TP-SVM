...
Dans cette question, nous étudions l’influence du paramètre de régularisation C
C sur les performances du SVM. Pour cela, nous testons différentes valeurs de C
C sur les images de deux individus du jeu de données et analysons l’évolution de l’accuracy en fonction de ce paramètre.
...
scores = []
for C in Cs:
    clf_tmp = SVC(kernel="linear", C=C, random_state=42)
...
# Réentraîner avec le meilleur C
...
plt.xscale("log")
plt.title("Influence de C sur les performances du SVM linéaire")
plt.grid(True)
plt.tight_layout()
plt.show()
...
# changement de la fonction pour qu'elle soit plus rapide
def run_svm_cv(_X, _y):
    _indices = np.random.permutation(_X.shape[0])
    _train_idx, _test_idx = _indices[:_X.shape[0] // 2], _indices[_X.shape[0] // 2:]
    _X_train, _X_test = _X[_train_idx, :], _X[_test_idx, :]
    ...
    _parameters = {'kernel': ['linear'], 'C': list(np.logspace(-3, 3, 5))}
    _svr = SVC()
    _clf_linear = GridSearchCV(_svr, _parameters, cv=3, n_jobs=-1)
    _clf_linear.fit(_X_train, _y_train)
...
On remarque que nous obtenons un score très médiocre d'environ 0,60, ce qui est plus faible qu'auparavant. En effet, ici pour n=5, la PCA compresse beaucoup l'information et est donc moins précise car elle projette dans un espace de dimension plus faible. 
En résultat, on a moins d'overfitting, mais également une moins bonne performance du fait d'une information moindre.
...
Pour un nombre de composantes plus grandes, 20 ici, on obtient à peine une meilleure performance. Ainsi, augmenter ce paramètre ne semble pas augmenter beaucoup nos résultats. De plus, il faut faire attention à ne pas trop augmenter ce nombre car après la PCA n'aura plus aucun intérêt puisqu'on se retrouvera avec le même modèle que sans. Il y a donc un compromis à faire entre réduction de dimension (moins de bruit) et préservation de l’information.
